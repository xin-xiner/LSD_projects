...
void KeyFrameGraph::addKeyFrame(Frame* frame)
{
 ...
 newKeyframesBuffer.push_back(frame);//wx- 由于直接加入g2o的图中需要等待g2oGraphAccessMutex，相当于需要等待优化线程完成，故使用buffer来完成添加，再在优化线程中添加入g2o图中//wx- 这个操作受到newConstraintMutex的保护
}
void KeyFrameGraph::insertConstraint(KFConstraintStruct* constraint)
{
 ...
 newEdgeBuffer.push_back(constraint);//wx- 由于直接加入g2o的图中需要等待g2oGraphAccessMutex，相当于需要等待优化线程完成，故使用buffer来完成添加，再在优化线程中添加入g2o图中//wx- 这个操作受到newConstraintMutex的保护
...
bool KeyFrameGraph::addElementsFromBuffer()
{
 ...
 for (Frame* newKF : newKeyframesBuffer)//newKeyframesBuffer中的元素是在FindConstrains线程中被添加进来的元素
 {
 ...
 newKeyframesBuffer.clear();//wx- 这个操作受到newConstraintMutex的保护
 for (auto edge : newEdgeBuffer)
 {
  graph.addEdge(edge->edge);
  added = true;
 }
 newEdgeBuffer.clear();//wx- 这个操作受到newConstraintMutex的保护
 return added;
}
