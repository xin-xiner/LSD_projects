...
{
    std::vector<TrackableKFStruct, Eigen::aligned_allocator<TrackableKFStruct> > potentialReferenceFrames =
            findEuclideanOverlapFrames(frame, maxScore / (KFDistWeight*KFDistWeight), 0.75);//wx- 利用距离选出可能的loop帧列
 ...
 for(unsigned int i=0;i<potentialReferenceFrames.size();i++)
 {
  if(frame->getTrackingParent() == potentialReferenceFrames[i].ref)
   continue;
  if(potentialReferenceFrames[i].ref->idxInKeyframes < INITIALIZATION_PHASE_COUNT)//wx- 剔除其中系统初始化时期的帧
   continue;
#ifdef TIME_CAL
  struct timeval tv_start, tv_end;
  gettimeofday(&tv_start, NULL);
#endif
  tracker->checkPermaRefOverlap(potentialReferenceFrames[i].ref, potentialReferenceFrames[i].refToFrame);//wx- 计算投影到当前帧范围内的点的加权个数
#ifdef TIME_CAL
