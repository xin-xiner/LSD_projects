...
//wx-记录了keyframeGraph中的一条边，保存了该边的两顶点之间的变换信息和变换的损失值
//wx-constraint在这里代表了连接两关键帧的边
...
public:
 ...
 std::vector< Frame*, Eigen::aligned_allocator<Frame*> > keyframesAll;//wx- 当SlamSystem 中 finishCurrentKeyframe中最终更新完当前关键帧之后，关键帧会加入这个队列//在调用完optimize之后在mappingThread里mergeOptimizationOffset中对这些关键帧中的姿态信息进行更新

 /** Maps frame ids to keyframes. Contains ALL Keyframes allocated, including the one that currently being created. */
 /* this is where the shared pointers of Keyframe Frames are kept, so they are not deleted ever *////wx- 我说怎么能都是指针又不会被释放掉呢
