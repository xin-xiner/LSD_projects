...
Frame::Frame(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image, const unsigned char* imageColor)
 :Frame(id,width,height,K,timestamp,image)
{
 if (imageColor == 0)
 {
  data.imageColor = 0;
 ...
 data.imageColor = (unsigned char*)(FrameMemory::getInstance().getBuffer(sizeof(unsigned char)*3 * data.width[0] * data.height[0]));
 unsigned char* maxPt = data.imageColor + (data.width[0] * data.height[0]*3);
 const unsigned char* colorData = imageColor;
 for (unsigned char *pt = data.imageColor; pt < maxPt; pt++, colorData++)
 {
  *pt = *colorData;
 }
}
...
Frame::~Frame()
{
 ...
 for (int level = 0; level < PYRAMID_LEVELS; ++ level)
 {
  ...
  FrameMemory::getInstance().returnBuffer(data.disField[level]);
  FrameMemory::getInstance().returnBuffer(data.disGrad[level]);
  FrameMemory::getInstance().returnBuffer(data.edgeFlag[level]);
 }
 ...
 FrameMemory::getInstance().returnBuffer(data.imageColor);
...
void Frame::setDepthFromGroundTruth(const float* depth, float cov_scale)
{
 ...
 const unsigned char* edge = edgeFlag(0);
...
void Frame::initialize(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp)
{
 ...
 for (int level = 0; level < PYRAMID_LEVELS; ++ level)
 {
  ...
  data.disFieldValid[level] = false;//wx-debug
  data.disGradValid[level] = false;//wx-debug
  data.edgeFlagValid[level] = false;
  data.edgeFlag[level] = 0;
  data.disField[level] = 0;//wx-debug
  data.disGrad[level] = 0;//wx-debug
...
void Frame::buildGradients(int level)
{
 ...
 const float* img_pt = data.image[level] + width;//wx-从第二行开始算起
  ...
  *(((float*)gradxyii_pt)+2) = val_00;//wx- 像素值，和*img_pt重复？
...
void Frame::buildMaxGradients(int level)
{
 ...
 Eigen::Vector4f* gradxyii_pt = data.gradients[level] + width;//wx-从第二行开始，是不是应该使用统一接口调用this->gradients(level)，以防止会有当前level梯度未生成的情况
  ...
  *maxgrad_pt = sqrtf(dx*dx + dy*dy);//wx -wtf这不就是梯度的大小么，叫maxgrad感觉不清晰
 }
 // 2. smear up/down direction into temp buffer //wx- 平滑梯度图，平滑的方式应该是取上下左右四个点的最大值
 ...
 {
  float g1 = maxgrad_pt[-width];//上面点的梯度值
  float g2 = maxgrad_pt[0];//当前点的梯度值
  if(g1 < g2) g1 = g2;
  float g3 = maxgrad_pt[width];//下面点的梯度值
  if(g1 < g3)
   *maxgrad_t_pt = g3;
  else
   *maxgrad_t_pt = g1;
 ...
 for(;maxgrad_pt<maxgrad_pt_max; maxgrad_pt++, maxgrad_t_pt++)
 {
  float g1 = maxgrad_t_pt[-1];//左边点
  float g2 = maxgrad_t_pt[0];//当前点
  if(g1 < g2) g1 = g2;
  float g3 = maxgrad_t_pt[1];//右边点
  if(g1 < g3)
  {
   *maxgrad_pt = g3;
...
//*************wx-debug*************
const float* Frame::disField(int level)
{
 if (!data.disFieldValid[level])
 {
  //require(MAX_GRADIENTS, level);
  edgeFlag(level);
  //if (data.disField[level] == 0)
   //data.disField[level] = (float*)FrameMemory::getInstance().getBuffer(sizeof(float) *  data.width[level] * data.height[level]);
  ////computeDistanceFieldFromGradient(data.maxGradients[level], data.width[level], data.height[level], data.disField[level]);
  //data.edgeCount[level] = computeDisFieldFromDepthFlag(data.edgeFlag[level], data.width[level], data.height[level], data.disField[level]);
  //data.disFieldValid[level] = true;
 }
 return data.disField[level];
}
const Eigen::Vector2f* Frame::disGrad(int level)
{

 if (!data.disGradValid[level])
 {
  //disField(level);//require disField
  //if (data.disGrad[level] == 0)
  // data.disGrad[level] = (Eigen::Vector2f*)FrameMemory::getInstance().getBuffer(sizeof(Eigen::Vector2f) *  data.width[level] * data.height[level]);
  //computeDistanceGradientsFromDisField(data.disField[level], data.width[level], data.height[level], data.disGrad[level]);
  //data.disGradValid[level] = true;
	 edgeFlag(level);
 }
 return data.disGrad[level];
}

const uchar* Frame::edgeFlag(int level)
{
 if (!data.edgeFlagValid[level])
 {
  if (data.edgeFlag[level] == 0)
   data.edgeFlag[level] = (uchar*)FrameMemory::getInstance().getBuffer(sizeof(uchar) *  data.width[level] * data.height[level]);
  if (data.disField[level] == 0)
   data.disField[level] = (float*)FrameMemory::getInstance().getBuffer(sizeof(float) *  data.width[level] * data.height[level]);
  if (data.disGrad[level] == 0)
   data.disGrad[level] = (Eigen::Vector2f*)FrameMemory::getInstance().getBuffer(sizeof(Eigen::Vector2f) *  data.width[level] * data.height[level]);
  //makeEdgeFlag(data.image[level], data.width[level], data.height[level], data.edgeFlag[level]);
  data.edgeCount[level] = computeDisFieldFromImage(data.image[level], data.width[level], data.height[level], data.edgeFlag[level], data.disField[level], data.disGrad[level],fileName);
  data.edgeFlagValid[level] = true;
  data.disFieldValid[level] = true;
  data.disGradValid[level] = true;
 }
 return data.edgeFlag[level];
}

const int Frame::edgeCount(int level)
{
 disField(level);
 return data.edgeCount[level];
}
//************

void Frame::freeTheDisFieldMemory()
{
 for (int level = 0; level < PYRAMID_LEVELS; ++level)
 {
  FrameMemory::getInstance().returnBuffer(data.disField[level]);
