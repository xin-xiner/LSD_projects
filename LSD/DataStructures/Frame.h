...
/**
 */
//用来保存像素值、梯度值、inversDepth、内参、帧编号等。
//通过一组内联函数返回不同level不同种类的数据，并在数据缺失时进行内存分配和计算
//将数据的获取封装到了属性函数里，统一将内存分配和读取操作推迟到了访问属性函数时进行。
//但深度计算部分在DepthMap类中
//除了图片读取既像素值的level0是直接获取
class Frame
{
public:
 ...
 Frame(int id, int width, int height, const Eigen::Matrix3f& K, double timestamp, const unsigned char* image,const unsigned char* imageColor);
 ...
 //wx-提供统一的接口访问数据，且当数据未计算时通过私有函数require计算。
  //wx-将对除0级像素信息外的图片信息的计算推迟到读取时进行。
 inline float* image(int level = 0);
 inline unsigned char* imageColor(){ return data.imageColor; };
 inline const Eigen::Vector4f* gradients(int level = 0);//返回梯度
 inline const float* maxGradients(int level = 0);//wx- 返回各像素的梯度值
 ...
 void takeReActivationData(DepthMapPixelHypothesis* depthMap);//wx-从depthMap的逆深度信息复制到当前帧的逆深度
 ...
 int numMappedOnThis;
 int numMappedOnThisTotal;//guo-keyframe被更新次数,5次分界//wx-记录currentKeyfram被更新了多少次，如果更新次数太少(MIN_NUM_MAPPED 5)就需要替换currentKeyframe则认为该keyframe无效，丢弃掉
 float meanIdepth;
 int numPoints;
 int idxInKeyframes;
...
private:
 ...
 void require(int dataFlags, int level = 0);//wx-用来生成某个level的数据
 ...
 //wx-build函数level0时分配内存，其他level从更低level创建
 void buildImage(int level);//wx-从原始图片生成对应level的图片
 void releaseImage(int level);
 
 void buildGradients(int level);//wx-从对应level的图片计算灰度的梯度
 void releaseGradients(int level);
 
 void buildMaxGradients(int level);
 void releaseMaxGradients(int level);
 
 void buildIDepthAndIDepthVar(int level);//wx-创建inverse Depth，level 0时分配内存，否则从上个level计算
 ...
 struct Data
 {
  ...
  //****************************wx-debug
  float* disField[PYRAMID_LEVELS];//wx-debug
  bool disFieldValid[PYRAMID_LEVELS];
  Eigen::Vector2f* disGrad[PYRAMID_LEVELS];//wx-debug
  bool disGradValid[PYRAMID_LEVELS];
  uchar* edgeFlag[PYRAMID_LEVELS];
  bool edgeFlagValid[PYRAMID_LEVELS];
  int edgeCount[PYRAMID_LEVELS];
 };
 Data data;
...
public:
 ...
 const float* disField(int level);
 
 const Eigen::Vector2f* Frame::disGrad(int level);
 const uchar* edgeFlag(int level);
 const int edgeCount(int level);
 void freeTheDisFieldMemory();
 std::string fileName;
};
