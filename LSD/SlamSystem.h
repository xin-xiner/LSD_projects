...
public:
 ...
 Undistorter *ptrUndistor;
 bool trackingIsGood;

 SlamSystem(int w, int h, Eigen::Matrix3f K, bool enableSLAM = true, Undistorter *undist = NULL);
 SlamSystem(const SlamSystem&) = delete;
 SlamSystem& operator=(const SlamSystem&) = delete;
 ~SlamSystem();
 void randomInit(uchar* image, double timeStamp, int id, const uchar* imageColor = 0, std::string filePath = "");
 void gtDepthInit(uchar* image, float* depth, double timeStamp, int id, const uchar* imageColor = 0, std::string imgpath = "");
 void provideNV21Image(unsigned char* data, int h, int w, int frmCnt);
 
 // tracks a frame.
 // first frame will return Identity = camToWord.
 // returns camToWord transformation of the tracked frame.
 // frameID needs to be monotonically increasing.
 void trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp, const uchar* imageColor = 0, float* depth = 0, std::string imgpath = "");
 ...
 void trackRGBDFrame(float* depth, const uchar* image, const uchar* imageColor, unsigned int frameID, bool blockUntilMapped, double timestamp);//wx-debug
 void trackFrameDisField(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp, const uchar* imageColor = 0);
#ifdef TIME_CAL
...
private:
 ...
 boost::mutex newConstraintMutex;//wx- 在findConstrains和optimization中保护两个线程对KeyFrameGraph中对newKeyframesBuffer和edgeBuffer的读写操作不冲突
 boost::condition_variable newConstraintCreatedSignal;
 boost::mutex g2oGraphAccessMutex;//保护optimization线程中对g2o图的操作不冲突，实际上当前仅优化线程和Slamsystem::OptimizeGraph会访问图，但OptimizeGraph未被使用
 ...
 boost::shared_mutex poseConsistencyMutex;//用来同步优化线程和mapping线程之间对图的读写的操作
 ...
 void finishCurrentKeyframe();//wx- 当关键帧要被替换、又不被丢弃时调用；或在所有帧计算完成时调用;  调用depthMap的finalizeKeyFrame,将depthMap中的深度数据传递到activeKeyFrame保存,将currentKeyFrame添加到keyFrameGraph中
 void discardCurrentKeyframe();//wx- 当一个关键帧刚添加，仅被更新很少次数就丢失track的话就丢弃该关键帧。\
          丢弃的操作为：检查是否已经是KFgraph中的帧(loopClosure)，如果是的话执行finalize。否则将depthmap设为invalid(currentKeyframe = 0)，并从KFgraph中的idToKey列表中删除
 void changeKeyframe(bool noCreate, bool force, float maxScore);//wx- 在createNewKeyFrame被置为true时调用,完成关键帧的替换操作\
                   用来替换关键帧，但注意这个函数完成了loopClosure检测，如果是loopClosure的话则替换成keyFrameGraph中loopClosure的帧\
                      noCreate用来控制是否进行mapping操作，force控制仅loopClosure时替换关键帧或者也可以拿最近的跟踪帧来替换关键帧
 void createNewCurrentKeyframe(std::shared_ptr<Frame> newKeyframeCandidate);//wx- 当更新关键帧时执行propagate & make new操作，但并不进行深度的重建。深度重建在updateKeyframe中
 void loadNewCurrentKeyframe(Frame* keyframeToLoad);//wx- 在changeKeyframe中被调用，如果找到了loopClosure\
                使用loop帧替换当前关键帧，并将depthMap置为loop帧\
                调用depthMap.setFromExistingKF\
                也在Retrack找到跟踪点时被调用
 bool updateKeyframe();//wx- 在createNewKeyFrame为false时调用; 从unmappedTrackedFrames中提取帧，构造reference队列; 调用map->updateKeyframe更新关键帧
 ...
 void testConstraint(
   ...
   float strictness);//wx- 这里是通过尝试给搜到的图中的可能相邻的节点计算边？
