...
UndistorterOpenCV::UndistorterOpenCV(const char* configFileName)
{
 ...
 K_.at<double>(2, 0) = (K_.at<double>(2, 0) / getOutputWidth()) * crop_width;
 K_.at<double>(2, 1) = (K_.at<double>(2, 1) / getOutputHeight()) * crop_height;
...
{
	cv::Mat rempaImage;
	cv::remap(image, rempaImage, map1, map2, cv::INTER_LINEAR);

 cv::Rect myROI((out_width - crop_width) / 2, (out_height - crop_height) / 2, crop_width, crop_height);
 //std::cout << out_width << " " << crop_width << " " << (crop_width - crop_width) / 2 << std::endl;
 //std::cout << myROI << std::endl; 
 rempaImage(myROI).copyTo(rempaImage);
 rempaImage.copyTo(result);
}
