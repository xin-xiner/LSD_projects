...
// {
 for (std::vector<Frame*, Eigen::aligned_allocator<lsd_slam::Frame*> >::iterator c = allKeyframesList.begin() ; 
  c != allKeyframesList.end() ; c++)
 {
 
  // insert
  //KFForReloc.push_back(allKeyframesList[k]);
  KFForReloc.push_back(*c);
...
void Relocalizer::threadLoop(int idx)
{
 ...
 while(continueRunning)
 {
  ...
  if(nextRelocIDX < maxRelocIDX && CurrentRelocFrame)
  {
   ...
   lock.unlock();
   // initial Alignment
   SE3 temp;//edit-by-wx 2015-10-30 由于直接传递SE3()NDK无法编译，顾先写成引用变量再传递给函数
   SE3& tempSim = temp;
   SE3 todoToFrame = tracker->trackFrameOnPermaref(todo, myRelocFrame.get(), tempSim);
   // try neighbours
   ...
   if(todoGoodVal > relocalizationTH)
   {
    ...
    for(Frame* nkf : todo->neighbors)
     ...
     /*for (std::unordered_set< Frame*, std::hash<Frame*>, std::equal_to<Frame*>,
      Eigen::aligned_allocator< Frame* > >::iterator c = todo->neighbors.begin(); c != todo->neighbors.end(); c++)
      Frame *nkf = *c;*/
    {
