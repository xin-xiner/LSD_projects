...
std::string MatVis::VisUcharArray(const uchar* v, int w, int h)
{
	lock();
	text.clear();
	//transfer.flush();
...
std::string MatVis::VisIdepth(const lsd_slam::DepthMapPixelHypothesis* v, int w, int h)
{
	lock();
	text.clear();
	//transfer.flush();
	...
	for (int i = 0; i < h; i++)
	{
		...
		for (int j = 0; j < w; j++)
		{
			transfer << std::left << std::setw(MAXSETW) << v[i*w + j].idepth;
		}
		transfer << std::endl;
	}
	transfer << " </pre>\n</html>" << std::endl;
...
std::string MatVis::VisVar(const lsd_slam::DepthMapPixelHypothesis* v, int w, int h)
{
	lock();
	text.clear();
	//transfer.flush();
	...
	for (int i = 0; i < h; i++)
	{
		...
		for (int j = 0; j < w; j++)
		{
			transfer << std::left << std::setw(MAXSETW) << v[i*w + j].idepth_var;
		}
		transfer << std::endl;
	}
	transfer << " </pre>\n</html>" << std::endl;
...
MatVis* MatVis::Vis()//getInstance()
{
	...
	if (instance == NULL)
	{
		...
		lsd_slam::DepthMapPixelHypothesis tempDepth[1] = { lsd_slam::DepthMapPixelHypothesis() };
		Vis()->VisIdepth(tempDepth, 1, 1);
		Vis()->VisVar(tempDepth, 1, 1);
